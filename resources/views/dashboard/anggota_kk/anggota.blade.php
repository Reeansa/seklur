                    ...
                    <form action="{{ route('data-kartu-keluarga.tambah-anggota') }}" method="post" class="p-5 space-y-5">
                        ...
                        <div>
                            <div class="flex gap-2 items-center w-full justify-between">
                                ...
                                <input type="hidden" name="no_kk" value="{{ $data_kartu_keluarga->no_kk }}">
                        ...
                        <div>
                            <div class="flex gap-2 items-center w-full justify-between">
                                <p>Alamat</p>
                                <input type="hidden" name="alamat" value="{{ $data_kartu_keluarga->alamat }}">
                                <input type="text" class="w-3/4 border border-black py-2 px-4"
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 items-center w-full">
                                ...
                                <select name="penduduk" id="" class="w-1/3 border border-black py-2 px-4">
                                    <option value="" class="hidden">-Pilih Penduduk-</option>
                                    @foreach ($penduduk as $p)
                                        <option value="{{ $p->nik }}">{{ $p->nama }}</option>
                                    @endforeach
                                </select>
                                <select name="hubungan_keluarga" id="" class="w-1/3 border border-black py-2 px-4">
                        ...
                        <tbody class="bg-[#ECECEC] border border-black">
                            @foreach ($anggota_keluarga as $ak)
                                <tr>
                                    ...
                                    <td class="border border-black">{{ $ak->hubungan_keluarga }}</td>
