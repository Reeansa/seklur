...
@section('content')
    <div class="grid grid-cols-12 w-full h-screen">
        ...
        <div class="col-span-9 bg-slate-orange-200 p-5 space-y-20 bg-slate-200">
            ...
            <form class="space-y-5 bg-white rounded-xl" action="{{ route('data-penduduk.update', $data_penduduk->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="p-5 space-y-5">
                    ...
                    <div>
                        <div class="flex gap-2 items-center w-full justify-between">
                            <p>Agama</p>
                            <input value="{{ $data_penduduk->agama }}" name="agama" type="text"
                                class="w-3/4 border border-black py-2 px-4">
                        </div>
                        @error('agama')
                            <p class="text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        <div class="flex gap-2 items-center w-full justify-between">
                            <p>Pekerjaan</p>
                            <input value="{{ $data_penduduk->pekerjaan }}" name="pekerjaan" type="text"
                                class="w-3/4 border border-black py-2 px-4">
                        </div>
                        @error('pekerjaan')
                            <p class="text-red-500">{{ $message }}</p>
